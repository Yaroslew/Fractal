...
{
	base->fract->mouse->re = (double)x / (base->width / (base->fract->max->re -
			base->fract->min->re)) + base->fract->min->re;
	base->fract->mouse->im = (double)y / (base->height / (base->fract->max->im -
			base->fract->min->im)) + base->fract->min->im;
}
